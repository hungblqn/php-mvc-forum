<?php
	...
	$sql = "SELECT * FROM BinhLuan join TaiKhoan on BinhLuan.idTaiKhoan = TaiKhoan.idTaiKhoan where idBaiViet = $idBaiViet ORDER BY idBinhLuan DESC";
	$result = $conn->query($sql);
            ...
            <div class="col-xl-12 left-box mt-4">
                <div class="card single-blog-post">
                    ...
                    <div class="body m-3">    
						...
						$result = $conn->query($sql);
						$icon = '<i class="mr-1 ti-heart"></i>';
						if (mysqli_num_rows($result) > 0) {
							// $icon = "<i class='mr-1 ti-face-sad'></i>";
							$icon = '<span  style="font-size:24px;" class="mr-1" >&#9829;</span>';
						}
						}
						...
						if(isset($_SESSION['idTaiKhoan'])){
							echo '<form method="post">
									<ul class="list-inline ml-2">
												...
												<li><button class="btn btn-sm reply-btn" data-target="#commentform"><i class="ml-3 mr-1 ti-comments"></i>Reply</button></li>
				...
				<?php
					foreach($binhluan as $bl){
						...
						$sql = "select * from TuongTacBinhLuan where  idBinhLuan = ".$bl['idBinhLuan']." and idTaiKhoan = ".$_SESSION['idTaiKhoan'] ;
						$result = $conn->query($sql);
...
<?php
	if(isset($_SESSION['email']) && $_SESSION['email'] != null){
		echo '<div class="container" id="commentform">
...
<script>
    // Xử lý sự kiện khi bấm vào nút "Reply"
    ...
    for (var i = 0; i < replyButtons.length; i++) {
        replyButtons[i].addEventListener('click', function(e) {
            ...
            if (target) {
                var inputField = document.querySelector(target + ' textarea');
                if (inputField) {
                    inputField.focus();
                }
            }
